...
from model.ops import deconv2d, conv2d
...
class GAN(BaseModel):
    ...
    def build_model(self):
        ...
        real_logit = self.discriminator(self.real_image, self.real_labels)

        # output of D for wrong caption/image
        wrong_logit = self.discriminator(self.real_image, fake_labels, reuse=True)

        # output of D for fake image
        self.fake_image = self.generator(noise, self.real_labels)
        fake_logit = self.discriminator(self.fake_image, self.real_labels, reuse=True)

        # ops for sampling an image
        sample_noise = tf.random_normal([1, 100])
        sample_label = tf.one_hot([1], 345)
        self.sample_image = self.generator(sample_noise, sample_label, reuse=True)
    ...
    def generator(self, noise, label, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            ...
            net = deconv2d(net, self.g_dim * 2, strides=[1, 1], padding='valid')  # (7 x 7 x 256)
            net = deconv2d(net, self.g_dim)  # (14 x 14 x 128)
    ...
    def discriminator(self, image, conditioning, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            net = conv2d(image, self.d_dim)  # (14 x 14 x 64)
            net = conv2d(net, self.d_dim * 2)  # (7 x 7 x 128)
            net = conv2d(net, self.d_dim * 4)  # (4 x 4 x 128)
